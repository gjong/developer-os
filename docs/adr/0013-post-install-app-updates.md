...
3. Ship **`update-extras.sh`** plus **`developer-os-update`**: resolve latest vfox (GitHub `releases/latest`) and Toolbox (JetBrains TBA products API), run **`flatpak update`**, re-run **`install-extras.sh`** with those versions (includes MacTahoe from git tip), and by default also **`pacman -Syu`**.
...
**Negative / trade-offs:** Needs network; “latest” for theme means git tip, not a pinned commit; JetBrains/GitHub APIs must stay reachable.
